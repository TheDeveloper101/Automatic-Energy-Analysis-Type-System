...
We also want the cost to be (actually) parametric in terms of the clock speed. What this means is that we introduce a new construct $\m{clock}$ which represents updating the device clock to the new speed. Rather than just representing cost expressions on their own, we instead represent them as $\Vec{f}.[C]$, where $\Vec{f}$ is the collection of clock speeds where the cost expression $C$ holds. 

We have the following grammar for defining cost expressions:
\[
\begin{array}{lcll}
     \text{Cost expression} & C  \bnfdef & C(p : st) & \text{(state cost)}  \\ & \bnfalt & C(p : (st, st')) & \text{(transition cost)} \\ & \bnfalt & C(p : (\alpha, st)) & \text{(non-concrete transition cost)} \\
     & \bnfalt & C(p : \alpha) & \text{(non-concrete cost)} \\ & \bnfalt & C_1 + C_2 & \text{(plus)} \\ & \bnfalt & C[\alpha \mapsto st] & \text{(concretization)}
\end{array}
\]
We treat cost expressions as symbolic undetermined expressions. $\m{cost}(\pctx) := \sum_{p \in \pctx} C(p : \alpha)$ aka the sum of all of the peripherals in their current state variables. $P$ is the collection of all of the cost expressions in our program under clock speeds $\Vec{f}$, which we refer to as the current cost property. $$P ::= \Vec{f}.[C], P' \mid \cdot$$. We write $P[\Vec{st}]$ to be the concretization of all of the $\alpha_i$ in $P$ with $\Vec{s}$ if possible. \sai{idk how to say this formally but it should apply to every cost expression seen so far} Also, we assume that $\m{max}(P_1, P_2)$ is well defined. Additionally, we define $P_1 \oplus P_2$ as follows, where we assume the existence of the function $\m{prefix}(P_1, P_2)$ that returns the common prefix of $P_1, P_2$
\begin{mathpar}
    \inferrule*[left=Property Join]
    {
    P_{pre} = \m{prefix}(P_1, P_2) \\ P_1 = P_{1-t}, P_{pre} \\ P_2 = P_{2-t}, P_{pre} \\ P_{1-t} = P_{1-f}, P_{1-i} \\ P_{2-t} = P_{2-f}, P_{2-i}  \\ P_{1-f} = \Vec{f_1}.[C_1] \\ P_{2-f} = \Vec{f_2}.[C_2] \\ \Vec{f} = \Vec{f_1} \cup \Vec{f_2} \\ P^* = \Vec{f}.[\cdot] \\ P = P^*, \m{max}(P_{1-t}, P_{2-t}), P_{pre}
    }{
    P_1 \oplus P_2 = P
    }
\end{mathpar}
...
\sai{$\pctx_1 \uplus \pctx_2$ is defined the exact same way from the state inference rules.}

\inferrule*[left=T-Clock]
{
f' \text{ fresh} \\ P' = f'.[\cdot], P
}
{
\tctx,\pctx, \Phi, P \vdash
\text{clock} :
...
{ \tctx, \pctx, \Phi, C \vdash x : \rho \\ \pctx(x) = \alpha_0 \\ \alpha_1 \text{ fresh} \\ \Phi' = \Phi \land \alpha_0 \leq st \land \alpha_1 = st \\ \pctx'[x] = \alpha_1 :: \pctx[x] \\ P = \Vec{f}.[C], P^* \\ P' = \Vec{f}.[C + C(p : (\alpha_0, st))], P^*
} 
{ 
...
{
\tctx, \pctx, \Phi, P \vdash x : \rho \\ s \hookrightarrow st \in x(D) \\ \pctx(x) = \alpha_0 \\ \alpha_1 \text{ fresh} \\ \Phi' = \Phi \land \alpha_0 = s \land \alpha_1 = st \\ \pctx'[x] = \alpha_1 :: \pctx[x] \\ P = \Vec{f}.[C], P^* \\ P' = \Vec{f}.[C + C(p : (\alpha_0, st))], P^*
}
{
\tctx,\pctx, \Phi, P \vdash
\text{downgrade}(x,st) :
...
{ \alpha_{\text{in}} \text{ fresh} \\ \pctx = p : [\alpha_{\text{i}}] \\ \tctx, \pctx, \cdot, \cdot \vdash c \rightarrow \pctx_f, \Phi_f, P'_f \\ \pctx_f(p) = \alpha_{\text{o}} \\ \Vec{st} = \m{solve}(\Phi_f \setminus \Phi_f|_{\alpha_i}) \\ P_f = P'_f[\Vec{st}] \\ \Vec{s_1} = \m{solve}(\Phi_f|_{\alpha_{\text{i}}}) \\ \Vec{s_2} = \m{solve}(\Phi_f|_{\alpha_{\text{o}}}) \\ \tau_f = \tau_1 \rightarrow \tau_2
} 
{ \tctx \vdash \text{fn } f(x, p) \ \{c; ret := e\} : \tau_f, P_f, \Phi_f|_{\alpha_i}, \Vec{s_1}, \Vec{s_2} }

\inferrule*[ left=T-Call]
{
\tctx(f)=
\tau_1 \rightarrow \tau_2, P_f, \Phi_f|_{\alpha_i}, \Vec{s_1}, \Vec{s_2}
\\ \alpha = \pctx(p) \\ \m{rename}(\Phi_f|_{\alpha_i}, \alpha) \\ P = \Vec{f}.[C], P^* \\
\tctx,\pctx,\Phi, P \vdash e : \tau_1, C_a \\ \Vec{f}.[C + C_a], P^* = P_1 \\ \alpha' \text{ fresh} \\ \Vec{s} = \m{solve}(\Phi_f|_{\alpha_i} \land \Phi|_{\alpha}) \\ \Vec{s} \subseteq \Vec{s_1} \\ \pctx'[p] = \alpha' :: \pctx[p] \\ P' = P_f[\Vec{s}], P_1 \\ \Phi' = \Phi \land \alpha \subseteq \Vec{s_1} \land \alpha' \subseteq \Vec{s_2} 
}
{
\tctx,\pctx, \Phi, P \vdash
\text{let } x = f(e,p) :
