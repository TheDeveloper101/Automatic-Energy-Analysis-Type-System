...
For atomic regions, we only care about satisfying the constraints on the possible peripheral states at the start of each region, so we do not need to have state inference rules.

We have the following additional inference contexts
\[
\begin{array}{lcll}
     \textit{Constraints} & \Phi & \bnfdef & \varphi, \Phi \bnfalt \cdot  \\
    \textit{State Constraints} & \varphi & \bnfdef & \alpha = \Vec{st} \bnfalt \varphi \leq st \bnfalt \varphi \hookrightarrow st \bnfalt \alpha \subseteq \Vec{st} \bnfalt \varphi_1 \wedge \varphi_2 \bnfalt \varphi_1 \vee \varphi_2 \\
...
\begin{aligned}
    \fbox{$\tctx, A, \Phi \vdash c \rightarrow \Phi'$}
\end{aligned}
$$

\begin{mathpar}
\inferrule*[left=T-Transition\(_\textit{inf}\)] 
{ \tctx, A, \Phi \vdash x : \rho \\ A(x) = \alpha \\ \Phi' = \Phi, \alpha \leq st
} 
{ 
\tctx, A,\Phi \vdash \text{transition}(x, st) : \text{unit} \rightarrow \tctx, \Phi' 
}

\inferrule*[left=T-Downgrade\(_\textit{inf}\)]
{
\tctx, A, \Phi \vdash x : \rho \\ A(x) = \alpha \\ \Phi' = \Phi, \alpha \hookrightarrow st
}
{ 
\tctx, A, \Phi \vdash \text{downgrade}(x, st) : \text{unit} \rightarrow \tctx, \Phi'
}

\inferrule*[left=T-If\(_\textit{inf}\)]
{
\tctx \vdash e : \text{bool} \\ \tctx, A, \Phi \vdash c_1 : \rightarrow  \Phi, \varphi_1 \\ \tctx, A, \Phi \vdash c_2 \rightarrow \Phi, \varphi_2 \\  \Phi^* = \Phi, \varphi_1 \vee \varphi_2 
}
{
\tctx, \pctx \vdash \text{if } e \text{ then } c_1 \text{ else } c_2: \tau \xrightarrow{C} \tctx^*, \Phi^*
}
\end{mathpar}
