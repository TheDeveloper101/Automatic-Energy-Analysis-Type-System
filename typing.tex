...
\text{Commands: }c & \bnfdef & e & \text{(expression)} \\ & \bnfalt & \text{let } x = e & \text{(let)} \\ 
  & \bnfalt &\text{let } x = f(e, p) & \text{(function call)} \\ & \bnfalt & \text{atomic}(c) & \text{(atomic)} \\
...
\end{array}
\]
\sai{maybe remove the command in the declarations part of the language? doesn't make sense to have sequences of commands, and then function declarations in between}
...
\begin{aligned}
    \fbox{$\tctx, \pctx, S \vdash c: \tau \Rightarrow \tctx', \pctx', S'$}
\end{aligned}
$$

Expression typing rules are of the form $$
\begin{aligned}
    \fbox{$\tctx, \pctx, S \vdash e: \tau $}
\end{aligned}
$$
