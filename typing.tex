...
\begin{mathpar}
    ...
    \inferrule*[left=Downgrade]
    {
    \Phi \vdash \alpha \hookrightarrow st
    }{
    \forall \alpha \in \Phi|_{\alpha}, \alpha \in p(D)
    }

\end{mathpar}
    \sai{for the downgrade rule im trying to say that if there is a constraint that $\alpha$ can downgrade to $st$, then the solution every $\alpha$ involved in constraints that reference this downgrade must be in the relevant downgrade policy.}
