...
We define [$\alpha$] to be a stack of type variables, with a lookup $A(p)$ referring to the top of it's stack,  $\alpha :: A(p)$ pushing $\alpha$ to the top of the stack and $A[p]$ to refer to the entire stack. 
...
\begin{mathpar}
    ...
    A_1(x) = \alpha_1 \\ A_2(x) = \alpha_2 \\ \alpha_1 \neq \alpha_2 \\ \alpha_3 \text{ fresh} \\ \Phi_3(x) = \Phi_1(x) \wedge \Phi_2(x) \wedge (\alpha_3 = \alpha_1 \vee \alpha_2) \\ \Phi_3' = \Phi_1' \oplus \Phi_2' 
    }{
    A_1, A_2 \vdash \Phi_1(x), \Phi_1' \oplus \Phi_2(x), \Phi_2' = \Phi_3(x), \Phi_3'
    }
\end{mathpar}
...
A_1(p) = \alpha_1  \\
A_2(p) = \alpha_2  \\
\alpha' \text{ fresh} \\ \alpha' = \alpha_1 \vee \alpha_2 \\ A' = A_1'[p] \uplus A_2'[p]
}
{
...
We define $\text{In}_f(\Vec{s}))$ to be the "weakest precondition" of $\Vec{s}$ \sai{what "weakest precondition" means is still tbd}. 

We also define $\Phi_f \models \alpha_{\text{out}}$ to be the concrete state assignment of $\alpha_{\text{out}}$ that satisfy $\Phi_f$ assuming such an assignment exists. \sai{not sure what an actual rigorous definition for this would be yet.}
...
{ \alpha_{\text{in}} \text{ fresh} \\ A_0 = p : [\alpha_{\text{in}}] \\ \tctx, A_0, \cdot \vdash c \rightarrow A_f, \Phi_f \\ A_f(p) = \alpha_{\text{out}} \\ \Vec{s} = \Phi_f \models \alpha_{\text{out}} \\ \tau_f = (\tau_1, \text{In}_f(\Vec{s})) \rightarrow (\tau_2, \Vec{s})
} 
{ \tctx \vdash \text{fn } f(x, p) \ \{c; ret := e\} : \tau_f \rightarrow A_f, \Phi_f}
