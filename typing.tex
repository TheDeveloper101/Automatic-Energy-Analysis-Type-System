...
\[
\begin{array}{lcll}
\text{Declarations: }d & \bnfdef & c & \text{(command)} \\ & \bnfalt & \text{atomic}(c) & \text{(atomic)} \\ & \bnfalt & \text{fn } f(x_{i}) : x_{r} \ \{c\} & \text{(function)} \\ & \bnfalt & d_1 \ d_2 & \text{(sequence)}
\end{array}
\]
\[
\begin{array}{lcll}
\text{Commands: }c & \bnfdef & e & \text{(expression)} \\ & \bnfalt & \text{let } x = e & \text{(let)} \\
  & \bnfalt &\text{let } x = f(e) & \text{(function call)} \\
...
\\ 
\textit{Types} & \tau & \bnfdef & \text{int} \bnfalt \text{bool} \bnfalt \tau_1 \rightarrow \tau_2  
\\
...
A downgrade policy $s' \hookrightarrow^q s$ means that a peripheral in state $s'$ can transition to state $s$ when condition $q$ is met, assuming that $s' > s$. A downgrade condition $q$ consists of a formula in proposition logic using base conditions $b$. Since we do not know what peripherals will be active when a function is declared, we extend our definition of $S$ to $S_f^{in/out}$ which maps possible peripherals to symbolic states. We assume the existence of two related functions: $\m{unify}(S, S_f)$ and $\m{max}(S)$. $\m{unify}(S, S_f)$ takes a symbolic possible state map $S_f$ and substitutes in values from $S$ as follows. \begin{mathpar}
   \inferrule{
    S' = \m{unify}(S, S_f) \\ \m{dom}(S) = \m{dom}(S_f)
   } {
   \m{unify}((p \mapsto s_p, S), (p \mapsto sym, S_f)) = p \mapsto s_p, S'
   }
\end{mathpar}
...
\(s_{i} \leq s_{j}\) and 
$\m{cost}(s_{i})$ = cost of being in state \(s_{i}\). Finally, $\m{cost}(\m{max}(S))$ = the sum of the cost of being in the maximum state for each peripheral. 
...
\todo{update the non-function rules} 
...
{
  \tctx,\pctx,S \vdash e : \xrightarrow{C_e} \tau_1, \pctx, S 
  \\
  \tctx(f) = (\tau_1, S_f^{in}) \xrightarrow{C_f} (\tau_2, S_f^{out}) \\
  S_c = \mathsf{unify}(S,\, S_f^{in}) \\ C = C_f + C_e + \m{cost} (\max (S_c) )
}
{
  \tctx, \pctx, S \vdash \text{let } x = f(e) : \tau_2, \xrightarrow{C} \tctx[x \mapsto \tau_2], \pctx', S_c
}
