...
\text{Commands: }c & \bnfdef & \text{let } x = e & \text{(let)} \\ 
  ...
  & \bnfalt & \text{if } e \text{ then } c_1 \text{ else } c_2 & \text{(branch)} \\ & \bnfalt & \text{transition}(x, \text{st}) & \text{(transition)} \\ & \bnfalt & \text{downgrade}(x, \text{st}) & \text{(downgrade)} \\ & \bnfalt & \text{clock}( new) & \text{(clock)} \\ & \bnfalt & c_1; c_2 & \text{(command seq)}
\end{array}
\]
\[
\begin{array}{lcll}
...
{ \tctx(x) = \tau }
{ \tctx,\pctx,f \vdash x : \tau, C_v(f) }


\inferrule*[left=T-BinOp]
{
\tctx,\pctx,f \vdash e_1 : \tau_1, C_1 \\
\tctx,\pctx,f \vdash e_2 : \tau_2, C_2 \\
\text{binop} : \tau_1 \times \tau_2 \to \tau
}
{
\tctx,\pctx,f \vdash e_1 \ \text{binop}\ e_2 :
\tau,
(C_1 + C_2)(f) 
}
...
{
\tctx,\pctx,f \vdash e : \tau, C_e \\ C = C_e(f) + C_l(f)
}
{
\tctx,\pctx,f \vdash
\text{let } x = e :
...
\tctx(x) = \rho \\
\pctx(x) = \alpha \\
\Vec{s} = \m{infer}(\alpha) \\ \forall i \in [0, \dots, n], s_i \leq st \\ C = \max(\m{cost}(s_i,st))(f)
}
{
\tctx,\pctx,f \vdash
\text{transition}(x,st) :
...
\tctx(x) = \rho \\
\pctx(x) = \alpha \\
\Vec{s} = \m{infer}(\alpha) \\ \forall i \in [0, \dots, n], s_i \hookrightarrow st \\ C = \max(\m{cost}(s_i,st))(f)
}
{
\tctx,\pctx,f \vdash
\text{downgrade}(x,st) :
...
{
\tctx[x:\tau_1],
\pctx[p:\Vec{s_1}], f \vdash c : \tau' \xrightarrow{C_1} \tctx', \pctx', f' \\ \pctx'(p) = \alpha_2 \\ \m{infer}(\alpha_2) = \Vec{s_2}\\ \tctx',\pctx',f' \vdash e : \tau_2, C_2 \\ C_f = C_1(\alpha_1, f) + C_2(\alpha_2, f)
}
{
\tctx,\pctx,f \vdash \text{fn } f(x,p): (\tau_1,\Vec{s_1}) \rightarrow (\tau_2,\Vec{s_2})
\{c; ret := e\} \xrightarrow \tctx[f \mapsto (\tau_1,\Vec{s_1}) \xrightarrow{\Phi_f} (\tau_2,\Vec{s_2})], \pctx,f
}
...
(\tau_2,\Vec{s_2})
\\
\tctx,\pctx,f \vdash e : \tau_1, C_a \\ \pctx(p) = \alpha \\ \Vec{s} = \m{infer}(\alpha) \\ \Vec{s} \subseteq \Vec{s_1} \\ C = C_f[\Vec{s}, f] + C_a
}
{
\tctx,\pctx,f \vdash
\text{let } x = f(e,p) :
